...
std::shared_ptr<Actor> SpartyFactory::Create(std::wstring imagesDir)
{
    ...
    auto katana = std::make_shared<KatanaDrawable>(L"Katana", imagesDir + L"/katana.png");
    katana->SetCenter(wxPoint(8, 8));
    katana->SetPosition(wxPoint(-240, 110));
    rarm->AddChild(katana);
